                ...
                if (sigsetjmp(segv_jmp_buf, 1) == 0) {
                    ...
                    *ptr_i = PATTERN_2;
                    ...
                    if (value_j_2 != PATTERN_2) {
                        ...
                        printf("  False positive: Page %zu (%p) and Page %zu (%p)\n",
                               i, addr_i, j, addr_j);
                        printf("    Test1: j=%016lx (expected %016lx)\n", 
                               value_j_1, PATTERN_1);
                        printf("    Test2: j=%016lx (expected %016lx) FAILED\n", 
                               value_j_2, PATTERN_2);
                    ...
                    *ptr_i = PATTERN_3;
                    ...
                    if (value_j_3 != PATTERN_3) {
                        ...
                        printf("  False positive: Page %zu (%p) and Page %zu (%p)\n",
                               i, addr_i, j, addr_j);
                        printf("    Test1: OK, Test2: OK\n");
                        printf("    Test3: j=%016lx (expected %016lx) FAILED\n", 
                               value_j_3, PATTERN_3);
                        pthread_mutex_unlock(&g_alias_mutex);
                        continue;
                    }
                    ...
                    // テスト4: 逆方向確認（jからi）
                    // ========================================
                    uint64_t original_value_j = *ptr_j;  // 現在はPATTERN_3のはず
                    ...
                    *ptr_j = PATTERN_REVERSE;
                    ...
                    if (value_i_reverse != PATTERN_REVERSE) {
                        // 一方向のみのエイリアス？（通常ありえない）
                        pthread_mutex_lock(&g_alias_mutex);
                        printf("  One-way alias?: Page %zu (%p) -> Page %zu (%p)\n",
                               i, addr_i, j, addr_j);
                        printf("    Forward tests: OK\n");
                        printf("    Reverse test: i=%016lx (expected %016lx) FAILED\n", 
                               value_i_reverse, PATTERN_REVERSE);
                        pthread_mutex_unlock(&g_alias_mutex);
                        continue;
                    }
