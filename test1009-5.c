...
// Alias candidate for verification
typedef struct {
    size_t index_i;
    size_t index_j;
    void *addr_i;
...
AliasCandidate *g_candidates = NULL;
size_t g_candidate_count = 0;
size_t g_candidate_capacity = 0;
pthread_mutex_t g_candidate_mutex = PTHREAD_MUTEX_INITIALIZER;

// =============================================================================
// Cache control functions
// =============================================================================

// Force flush cache line to physical memory
static inline void flush_cache_line(void *addr) {
    _mm_clflush(addr);
    _mm_mfence();  // Memory barrier
}

// Force read from physical memory (invalidate cache first)
static inline uint64_t read_from_physical(void *addr) {
...
static inline void write_to_physical(void *addr, uint64_t value) {
...
// =============================================================================
// Alias candidate management
// =============================================================================

void init_candidates() {
    g_candidate_capacity = 1000;
    g_candidates = malloc(sizeof(AliasCandidate) * g_candidate_capacity);
    g_candidate_count = 0;
}

void add_candidate(size_t i, size_t j, void *addr_i, void *addr_j) {
    pthread_mutex_lock(&g_candidate_mutex);
    
    if (g_candidate_count >= g_candidate_capacity) {
        g_candidate_capacity *= 2;
        g_candidates = realloc(g_candidates, 
                               sizeof(AliasCandidate) * g_candidate_capacity);
    }
    
    g_candidates[g_candidate_count].index_i = i;
    g_candidates[g_candidate_count].index_j = j;
    g_candidates[g_candidate_count].addr_i = addr_i;
    g_candidates[g_candidate_count].addr_j = addr_j;
    g_candidate_count++;
    
    pthread_mutex_unlock(&g_candidate_mutex);
}

// =============================================================================
// Two-stage alias verification
// =============================================================================

int verify_alias_pair(void *addr_i, void *addr_j) {
    // Test pattern sequence for comprehensive verification
    ...
    uint64_t original_i = read_from_physical(addr_i);
    uint64_t original_j = read_from_physical(addr_j);
    
    for (int test = 0; test < num_tests; test++) {
        uint64_t pattern = test_patterns[test];
        
        // Step 1: Write pattern to addr_i
        write_to_physical(addr_i, pattern);
        
        // Step 2: Read from addr_j (should see the pattern if aliased)
        uint64_t value_j = read_from_physical(addr_j);
        
        if (value_j == pattern) {
            // Step 3: Verify reverse direction
            // Write different pattern to addr_j
            uint64_t reverse_pattern = ~pattern;
            write_to_physical(addr_j, reverse_pattern);
            
            // Step 4: Read from addr_i (should see reverse pattern if aliased)
            uint64_t value_i = read_from_physical(addr_i);
            
            if (value_i == reverse_pattern) {
                passed_tests++;
            }
        }
    }
    ...
    write_to_physical(addr_i, original_i);
    write_to_physical(addr_j, original_j);
    
    // Consider it confirmed if at least 80% of tests passed
    return (passed_tests >= (num_tests * 4 / 5));
}
...
// =============================================================================
// Stage 1: Initial candidate detection with cache flushing
// =============================================================================
...
void *alias_worker(void *arg) {
    ...
    for (size_t i = data->start_index; i < data->end_index; i++) {
        ...
        // Save original value with cache flush
        uint64_t original_i = read_from_physical(addr_i);
        
        // Write test pattern to physical memory
        write_to_physical(addr_i, TEST_PATTERN);
        ...
        for (size_t j = i + 1; j < block_count; j++) {
            ...
            // Read from physical memory (bypass cache)
            uint64_t value_j = read_from_physical(addr_j);
            
            if (value_j == TEST_PATTERN) {
                // Potential alias found - add to candidate list
                add_candidate(i, j, addr_i, addr_j);
            }
        }
...
void stage3_check_aliases() {
    printf("\n=== Stage 3: Check Aliases (Two-Stage Verification) ===\n");
    ...
    // Phase 1: Quick scan with cache flushing
    printf("\n--- Phase 1: Initial Scan with Cache Flushing ---\n");
    ...
    printf("Phase 1 complete: Found %zu candidate pair(s)\n", g_candidate_count);

    if (g_candidate_count == 0) {
        printf("\nResult: No aliases found\n");
        return;
    }

    // Phase 2: Comprehensive verification of candidates
    printf("\n--- Phase 2: Comprehensive Verification ---\n");
    printf("Verifying %zu candidate(s) with multiple test patterns...\n", 
           g_candidate_count);
    ...
    for (size_t i = 0; i < g_candidate_count; i++) {
        AliasCandidate *candidate = &g_candidates[i];
        
        printf("\nVerifying candidate %zu/%zu:\n", i + 1, g_candidate_count);
        printf("  Page %zu (%p) <-> Page %zu (%p)\n",
               candidate->index_i, candidate->addr_i,
               candidate->index_j, candidate->addr_j);
        
        if (verify_alias_pair(candidate->addr_i, candidate->addr_j)) {
            confirmed++;
            printf("  ✓ CONFIRMED: This is a genuine alias pair\n");
            
            // Get physical addresses for confirmation
            uint64_t phys_i = get_physical_address(candidate->addr_i);
            uint64_t phys_j = get_physical_address(candidate->addr_j);
            
            if (phys_i > 0 && phys_j > 0) {
                printf("    Physical address i: 0x%lx\n", phys_i);
                printf("    Physical address j: 0x%lx\n", phys_j);
                printf("    Difference: 0x%lx\n", 
                       phys_i > phys_j ? phys_i - phys_j : phys_j - phys_i);
            }
            
            display_count++;
            if (display_count >= 10 && i < g_candidate_count - 1) {
                if (ask_user("Continue verifying remaining candidates?")) {
                    display_count = 0;
                } else {
                    printf("\nStopping verification. Remaining: %zu\n", 
                           g_candidate_count - i - 1);
                    break;
                }
            }
        } else {
            printf("  ✗ FALSE POSITIVE: Not a genuine alias\n");
        }
    }

    printf("\n=== Final Result ===\n");
    printf("Candidates found: %zu\n", g_candidate_count);
    printf("Confirmed aliases: %d\n", confirmed);
    
    if (confirmed > 0) {
        printf("\n*** BadRAM CONFIRMED ***\n");
        printf("Physical memory aliasing detected with high confidence.\n");
        printf("Multiple test patterns confirmed the aliasing relationship.\n");
    } else if (g_candidate_count > 0) {
        printf("\nAll candidates were false positives.\n");
        printf("No genuine aliases detected.\n");
    } else {
        printf("\nNo aliases found.\n");
    }
...
int main() {
    ...
    printf("  Two-Stage Alias Verification\n");
    printf("========================================\n\n");
    
    printf("Features:\n");
    printf("  - Cache flush (clflush + mfence)\n");
    printf("  - Phase 1: Quick scan for candidates\n");
    printf("  - Phase 2: Multi-pattern verification\n");
    printf("  - Bidirectional alias confirmation\n\n");
