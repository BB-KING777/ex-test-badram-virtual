...
// ★★★ 最大ブロック数を定義 ★★★
#define MAX_BLOCKS 3000000  // 約3百万ページ = 12GB分
...
// Global variables
// ★★★ 静的配列に変更 ★★★
MemBlock mem_blocks_static[MAX_BLOCKS];
...
void stage1_allocate_memory() {
    ...
    for (size_t i = 0; i < max_pages; i++) {
        ...
        // ★★★ block_count が実際に増えているか確認 ★★★
        if (block_count != i + 1) {
            printf("ERROR: block_count mismatch at iteration %zu (expected %zu, got %zu)\n",
                   i, i + 1, block_count);
            break;
        }
...
void stage2_write_pages() {
    ...
    // ★★★ 整合性チェックを追加 ★★★
    printf("\n=== Verifying mem_blocks integrity before writing ===\n");
    size_t invalid_count = 0;
    for (size_t i = 0; i < block_count; i++) {
        if (mem_blocks[i].addr == NULL) {
            printf("ERROR at index %zu: addr is NULL\n", i);
            invalid_count++;
            if (invalid_count >= 10) {
                printf("... (stopping after 10 errors)\n");
                break;
            }
        } else if ((uintptr_t)mem_blocks[i].addr < 0x1000) {
            printf("ERROR at index %zu: addr is too low: %p\n", 
                   i, mem_blocks[i].addr);
            invalid_count++;
            if (invalid_count >= 10) break;
        } else if (mem_blocks[i].size != PAGE_SIZE) {
            printf("ERROR at index %zu: size is wrong: %zu (expected %d)\n",
                   i, mem_blocks[i].size, PAGE_SIZE);
            invalid_count++;
            if (invalid_count >= 10) break;
        }
        
        if ((i + 1) % 50000 == 0) {
            printf("  Verified %zu / %zu entries...\n", i + 1, block_count);
        }
    }
    
    if (invalid_count > 0) {
        printf("\n!!! Found %zu invalid entries !!!\n", invalid_count);
        printf("mem_blocks array has been corrupted!\n");
        return;
    }
...
int main() {
    printf("Memory Address Verification Program (Static Array Version)\n");
    printf("================================================================\n");
    printf("MAX_BLOCKS: %d (%.2f GB)\n\n", MAX_BLOCKS, (double)(MAX_BLOCKS * PAGE_SIZE) / GB);
    ...
    // ★★★ 静的配列なので mem_blocks 自体は free しない ★★★
