        ...
        if (sigsetjmp(segv_jmp_buf, 1) == 0) {
            ...
            for (size_t idx_j = idx_i + 1; idx_j < data->valid_count; idx_j++) {
                ...
                if (sigsetjmp(segv_jmp_buf, 1) == 0) {
                    ...
                    // ========================================
                    // Algorithm 1 (論文 p.6) の実装
                    // ========================================
                    
                    // ランダムな2つの値を生成
                    uint64_t m1 = ((uint64_t)rand() << 32) | rand();
                    uint64_t m2 = ((uint64_t)rand() << 32) | rand();
                    ...
                    *ptr_i = m2;
                    ...
                    if (xor_written == xor_read && xor_written != 0) {
                        ...
                        printf("\n=== ALIAS DETECTED (XOR Method) ===\n");
                        printf("  Page %zu: Virtual=%p\n", i, addr_i);
                        printf("  Page %zu: Virtual=%p\n", j, addr_j);
                        printf("  XOR Test Results:\n");
                        printf("    Written to A: m1=%016lx, m2=%016lx\n", m1, m2);
...
void stage3_check_aliases() {
    printf("\n=== Stage 3: Check Aliases (XOR Method) ===\n");
    ...
    printf("Using XOR-based detection method (Algorithm 1 from paper)\n");

    // ========================================
    // マルチスレッドでエイリアススキャン（XOR版）
    // ========================================
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        pthread_create(&threads[i], NULL, alias_worker_v2_xor, &thread_data[i]);
    }
    ...
    printf("\n=== Scan Complete ===\n");
    if (g_pairs_found == 0) {
        printf("Result: No aliases found\n");
        printf("Note: This could mean:\n");
        printf("  - No BadRAM present (system is healthy)\n");
        printf("  - Memory aliasing exists but not in malloc() range\n");
    } else {
        printf("Result: %d alias pair(s) confirmed\n", g_pairs_found);
        printf("\n*** BadRAM CONFIRMED ***\n");
        printf("Physical memory aliasing detected using XOR method.\n");
        printf("This indicates SPD manipulation or hardware fault.\n");
    }
}
// =============================================================================
// Stage 4
